...
class NativeInterface:
    ...
    async def read_access(self, dut, addr):
        await RisingEdge(dut._id(self.name['clock'],extended=False))
        dut._id(self.name['ren'],extended=False).value = 1
        dut._id(self.name['raddr'],extended=False).value = addr
        await RisingEdge(dut._id(self.name['clock'],extended=False))
        dut._id(self.name['ren'],extended=False).value = 0
        await RisingEdge(dut._id(self.name['rvalid'],extended=False))
        await FallingEdge(dut._id(self.name['clock'],extended=False))
        data = int(dut._id(self.name['rdata'],extended=False).value)
        await RisingEdge(dut._id(self.name['clock'],extended=False))
        return data
