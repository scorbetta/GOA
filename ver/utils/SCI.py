    ...
    async def start_slave(self, dut, addr_lens, data_lens):
        ...
        while 1:
            ...
            if wnr == 1:
                ...
                #@DBUGprint(f'dbug: Write done at addr={addr},data={data},pid={pid},mems={self.mems[pid]}')
            else:
                # Read
                ...
                #@DBUGprint(f'dbug: Reading from addr={addr},pid={pid},mems={self.mems[pid]}')
            ...
            # Wait for the Master to release chip-select
            while 1:
                await RisingEdge(dut._id(self.name['clock'],extended=False))
                await FallingEdge(dut._id(self.name['clock'],extended=False))
                if int(dut._id(self.name['csn'],extended=False).value) == self.all_1s:
                    break
